...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.integration.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.integrations.update", [$integration->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="key">{{ trans('cruds.integration.fields.key') }}</label>
                <input class="form-control {{ $errors->has('key') ? 'is-invalid' : '' }}" type="text" name="key" id="key" value="{{ old('key', $integration->key) }}" required>
                @if($errors->has('key'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('secret') ? 'is-invalid' : '' }}" type="text" name="secret" id="secret" value="{{ old('secret', $integration->secret) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id">
                    @foreach($users as $id => $user)
                        <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $integration->user->id ?? '') == $id ? 'selected' : '' }}>{{ $user }}</option>
                    @endforeach
                </select>
                @if($errors->has('user'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('platform') ? 'is-invalid' : '' }}" name="platform_id" id="platform_id" required>
                    @foreach($platforms as $id => $platform)
                        <option value="{{ $id }}" {{ (old('platform_id') ? old('platform_id') : $integration->platform->id ?? '') == $id ? 'selected' : '' }}>{{ $platform }}</option>
                    @endforeach
                </select>
