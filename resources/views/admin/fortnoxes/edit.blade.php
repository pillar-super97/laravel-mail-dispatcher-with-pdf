...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.fortnox.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.fortnoxes.update", [$fortnox->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id" required>
                    @foreach($users as $id => $user)
                        <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $fortnox->user->id ?? '') == $id ? 'selected' : '' }}>{{ $user }}</option>
                    @endforeach
                </select>
                @if($errors->has('user'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('client') ? 'is-invalid' : '' }}" type="text" name="client" id="client" value="{{ old('client', $fortnox->client) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('secret') ? 'is-invalid' : '' }}" type="text" name="secret" id="secret" value="{{ old('secret', $fortnox->secret) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('access_code') ? 'is-invalid' : '' }}" type="text" name="access_code" id="access_code" value="{{ old('access_code', $fortnox->access_code) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('access_token') ? 'is-invalid' : '' }}" type="text" name="access_token" id="access_token" value="{{ old('access_token', $fortnox->access_token) }}">
